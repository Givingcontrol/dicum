    ...
    def __init__(self, *args, **kwargs):
        ...
        self.setStyleSheet(
            "QPushButton { font-family: Love Story Rough;} :enabled { color: white; border-image:url(" + os.path.join(
                ...
                ConfigManager().ICONS, "pc_red_spades.jpg") + "); }")
...
class MainGameWidget(QWidget):
    def __init__(self):
        ...
        self.deque = DequeManager(ConfigManager().DEQUE_FILE)
        ...
        self.lock_counter_widget = LockCounterWidget(
            current_locked=self.lock_counter, current_unlocked=self.unlock_counter)
    ...
    def __load_lock_status(self):
        with open(ConfigManager().LOCK_STATUS_FILE) as file:
    ...
    def draw_card(self, pos):
        ...
        if card.kind == "green":
            ...
            if self.unlock_counter >= ConfigManager().UNLOCK_LIMIT:
            ...
            self.view.setHtml(self.generator.get_task_view(), QUrl(ConfigManager().HTML_REL_PATH))
        else:
    ...
    def show_welcome_screen(self):
        if self.time_restrictor.is_restricted():
            ...
            self.view.setHtml(self.generator.get_unrestricted(), QUrl(ConfigManager().HTML_REL_PATH))
    ...
    def store_lock_status(self):
        with open(ConfigManager().LOCK_STATUS_FILE, "w") as file:
